package com.eastcom.common.shiro.session;
...
public class JedisSessionDAO extends AbstractSessionDAO implements SessionDAO {
	...
	public void update(Session session) throws UnknownSessionException {
		...
		if (request != null) {
			...
			// 如果是视图文件，则不更新SESSION
			if (StringUtils.startsWith(uri, PropertyUtils.getConfig("web.view.prefix")) && StringUtils.endsWith(uri, PropertyUtils.getConfig("web.view.suffix"))) {
				return;
			}
			// 手动控制不更新SESSION
			if (Constants.YesOrNo.NO.value().equals(request.getParameter("updateSession"))) {
				return;
			}
		}
		...
		try {
			...
			jedis.hset(sessionKeyPrefix, session.getId().toString(), principalId + "|" + session.getTimeout() + "|"
