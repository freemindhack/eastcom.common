...
public class FileUtils extends org.apache.commons.io.FileUtils {
	 ...
	 *            目标zip文件
	 */
	public static void zipFiles(String srcDirName, String[] fileNames, String descFileName) {
		// 判断目录是否存在
		if (srcDirName == null) {
			log.debug("文件压缩失败，目录 " + srcDirName + " 不存在!");
			return;
	...
	public static String getAbsolutePath(String urlPath) {
		if (StringUtils.isBlank(urlPath)) {
			...
			return SystemUtils.getRootPath() + urlPath.substring(urlPath.indexOf("/", 1), urlPath.length());
		}
	}
	 ...
	 * @throws IOException
	 */
	public static boolean writeBytes(String filePath, byte[] content) throws IOException {
	 ...
	 * @throws IOException
	 */
	public static boolean renameFile(String path, String suffix) throws IOException {
		renameFile(new File(path), suffix);
		return true;
	}
	 ...
	 * @throws IOException
	 */
	public static boolean renameFile(File file, String suffix) throws IOException {
		if (!file.exists()) {
			return false;// 重命名文件不存在
		}
		String oldName = file.getName();
		String newName = oldName.replace(oldName.substring(oldName.indexOf(".")), suffix);
